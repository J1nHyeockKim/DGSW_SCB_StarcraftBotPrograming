...
public class StrategyManager {
	...
	private int playTypeMem = 0;
	Random playType = new Random();
	...
	public void update() {
		...
		if(playTypeMem == 1) {
			if(GameCommander.createdWorkerCount >= 24 && timing == 1) {
				WorkerManager.nonGasTiming = false;
				timing = 2;
			}
			else if(GameCommander.createdZealotCount >= 24 && timing == 2) {
				WorkerManager.nonGasTiming = true;
				timing = 3;
			}
			else if(GameCommander.createdZealotCount >= 32 && timing == 3) {
				isFullScaleAttackStarted = true;
				timing = 4;
			}
			else if(timing == 4 && GameCommander.createdZealotCount >= 50) {
				isFullScaleAttackStarted = false;
				timing = 5;
			}
			else if(timing == 5 && GameCommander.createdZealotCount >= 70) {
				isFullScaleAttackStarted = true;
				timing = 6;
			}
		}
		else {
			if(timing == 1 && GameCommander.createdZealotCount >= 18) {
				WorkerManager.nonGasTiming = true;
				Config.WorkersPerRefinery = 0;
				timing = 2;
			}
			else if(timing == 2 && GameCommander.createdZealotCount >= 36) {
				isFullScaleAttackStarted = true;
				timing = 3;
			}
			else if(timing == 3 && GameCommander.createdZealotCount >= 50) {
				isFullScaleAttackStarted = false;
				timing = 4;
			}
			
			else if(timing == 4 && GameCommander.createdZealotCount >= 70) {
				isFullScaleAttackStarted = true;
				timing = 5;
			}
		}	
	...
	public void setInitialBuildOrder() {
		if (MyBotModule.Broodwar.self().getRace() == Race.Zerg) {
			...
			BuildManager.Instance().buildQueue.queueAsLowestPriority(InformationManager.Instance().getWorkerType());
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Zerg_Overlord);
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UpgradeType.Grooved_Spines);
	...
	public void executeCombat() {
		...
		if (isFullScaleAttackStarted == false) {
			Chokepoint SecondChokePoint = BWTA.getNearestChokepoint(InformationManager.Instance().getMainBaseLocation(InformationManager.Instance().selfPlayer).getTilePosition());
			...
			if (MyBotModule.Broodwar.self().completedUnitCount(UnitType.Zerg_Hydralisk) >= 12) {
